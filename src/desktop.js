...
function createTrayIcon() {
  if (process.platform === 'darwin') {
    const source = nativeImage.createFromPath(getBuildAssetPath('icon.png'));
    const image = (source.isEmpty() ? createAppIcon() : source).resize({ width: 18, height: 14 });
...
function buildTray() {
  ...
  const menu = Menu.buildFromTemplate([
    ...
    { type: 'separator' },
    {
      label: 'Salir de Ventysfy Print Agent',
      click: () => quitApp().catch((error) => {
